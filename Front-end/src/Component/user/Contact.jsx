...
const Contact = () => {
  ...
  const validationRules = {
    name: {
      ...
      validate: (value) => value.trim().length >= 2 || "Name cannot be only spaces"
    },
    email: {
    ...
    phone: {
      pattern: {
        value: /^[6-9]\d{9}$/,
        message: "Must be exactly 10 digits starting with 6-9",
      }
    },
    subject: {
      ...
      validate: (value) => value.trim().length >= 5 || "Subject cannot be only spaces"
    },
    message: {
      ...
      validate: (value) => value.trim().length >= 10 || "Message cannot be only spaces"
    },
  };
            ...
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-gray-100 h-full relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div className="flex flex-col">
                      <label className="text-gray-700 font-bold text-sm mb-2 ml-1">Phone Number (Optional)</label>
                      <input
                  ...
                  <button 
                    disabled={!isValid}
                    className={`w-full text-white font-black py-4 rounded-2xl transition-all duration-300 transform flex items-center justify-center gap-3 text-lg mt-2 ${!isValid ? 'bg-gray-400 cursor-not-allowed opacity-70' : 'bg-linear-to-r from-green-500 to-emerald-600 hover:shadow-xl hover:shadow-green-500/30 hover:-translate-y-1'}`}
                  >
